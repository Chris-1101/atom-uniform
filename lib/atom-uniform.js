...
export default {
  ...
  activate() {
    ...
    this.uniformView.miniEditor.element.addEventListener('blur', this.close.bind(this));
    ...
    this.subscriptions = new CompositeDisposable(
      ...
      atom.commands.add(this.uniformView.miniEditor.element, 'core:confirm', () => {
        this.process();
      }),
      atom.commands.add(this.uniformView.miniEditor.element, 'core:cancel', () => {
        this.close();
      })
    );
  },
  ...
  open() {
    ...
    this.uniformView.message.textContent = 'Insert text to be formatted:';
    this.uniformView.miniEditor.element.focus();
  },

  process() {
    const text = this.uniformView.miniEditor.getText();
    const format = Format.BOLDITALIC; // TODO retrieve this from modalPanel
  ...
  convert() {
    // TODO: CTRL+ALT+U, CTRL+< vim-mode-plus.normal
  },
